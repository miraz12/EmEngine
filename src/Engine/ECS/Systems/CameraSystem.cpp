...
void CameraSystem::updateMatrices(std::shared_ptr<CameraComponent> camera) {
  ...
  camera->m_ProjectionMatrix = glm::perspective(
      glm::radians(camera->m_fov), camera->m_width / camera->m_height,
      camera->m_near, camera->m_far);

  camera->m_matrixNeedsUpdate = false;
}

void CameraSystem::bindProjViewMatrix(std::shared_ptr<CameraComponent> camera,
                                      u32 proj, u32 view) {
  if (camera->m_matrixNeedsUpdate) {
    updateMatrices(camera);
  }
  glUniformMatrix4fv(proj, 1, GL_FALSE,
                     glm::value_ptr(camera->m_ProjectionMatrix));
  glUniformMatrix4fv(view, 1, GL_FALSE, glm::value_ptr(camera->m_viewMatrix));
}

void CameraSystem::bindProjMatrix(std::shared_ptr<CameraComponent> camera,
                                  u32 proj) {
  if (camera->m_matrixNeedsUpdate) {
    updateMatrices(camera);
  }
...
void CameraSystem::bindViewMatrix(std::shared_ptr<CameraComponent> camera,
                                  u32 view) {
  if (camera->m_matrixNeedsUpdate) {
    updateMatrices(camera);
  }
